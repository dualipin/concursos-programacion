...
usuarioRuta.post("/registrar", async (req, res) => {
  const { rfc, contra, tipo, nom, apds, fnac, sex, correo, tel } = req.body
...
usuarioRuta.put("/actualizar/:rfc", async (req, res) => {
  ...
  const { contra, tipo, nom, apds, fnac, sex, correo, tel } = req.body
  ...
  try {
    ...
    if (contra) {
      ...
      await cone.query("UPDATE usuario SET contra = ? WHERE rfc = ?", [
        hashedPassword,
        rfc,
      ])
    }

    await cone.query("UPDATE usuario SET tipo = ? WHERE rfc = ?", [tipo, rfc])
...
usuarioRuta.get("/obtener", async (_req, res) => {
  try {
    ...
    usuarios.forEach((usuario: any) => {
      usuario.fnac = parseDate(usuario.fnac)
    })
...
usuarioRuta.get("/obtener/:rfc", async (req, res) => {
  ...
  try {
    ...
    res.json({ ...usuarioRows[0], fnac: parseDate(usuarioRows[0].fnac) })
  } catch (error) {
